...
with_preserve_seed(dist <- vegdist(comp, method = "bray"))  

# can use PCA = vegan::rda with no covariates
# pcoa_ord <- pcoa(dist)  
with_preserve_seed(nmds_ord <- metaMDS(dist))

saveRDS(nmds_ord, here::here("data", "ordination.rds"))
