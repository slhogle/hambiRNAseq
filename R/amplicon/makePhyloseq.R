...
# read data
counts.metadata = read_tsv(here("data", "ampliconMetadata.tsv"), col_types = "cfffff")
counts = read_tsv(here("data", "speciesCounts.tsv"), col_types = "ccccd")

# make otu matrix
otumat = left_join(counts, counts.metadata) %>%
...
saveRDS(physeq, file=here::here("data", "physeq.rds"))
