...
shannonDivNet = readRDS(here::here("data", "shannonDivNet.rds")) 
shannonPlugin = readRDS(here::here("data", "shannonPlugin.rds"))
...
# Fig. 3B -----------------------------------------------------------------
# NMDS spider plot
nmds_ord = readRDS(here::here("data", "ordination.rds"))
metadf = read_tsv(here("data", "ampliconMetadata.tsv"), col_types = "cfffff") %>%
...
library(corncob)
modInteraction = readRDS(here::here("data", "corncobInteraction.rds"))
modPredation   = readRDS(here::here("data", "corncobPredation.rds"))
modEvolution   = readRDS(here::here("data", "corncobEvolution.rds"))
...
pf = readRDS(here::here("data", "corncobSBW25.rds")) 

pf.qm = pf[[2]]
pf.dat = pf[[1]]$dat

p.sbw25 <- ggplot(pf.qm) + 
  geom_jitter(data=pf.dat, aes(x=predation, color=expcombo, y=W/M), size=0.5,
              position=position_dodge2(width=0.5)) +
...
# Make final figure
library(patchwork)
pfinal <- (p.shannon / p.sbw25 / p.cccf) | p.nmds + plot_layout(guides = "collect")
#pfinal
