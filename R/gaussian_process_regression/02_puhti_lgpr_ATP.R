...
fit <- lgp(y ~ gp(day) + zs(id)*gp(day) + zs(pred)*gp(day) + 
               zs(evo)*gp(day) + zs(pred_evo)*gp(day) + gp_vm(recoveryonset),
             lgpinput,
             likelihood = "gaussian",
             prior = my_prior,
             verbose = TRUE,
...
write_rds(fit, "output/lgpr_fit_atp_xz.rds", "xz", compression = 9L)