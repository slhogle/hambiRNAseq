...
```{r}
pfanc =
  read_tsv(here::here("variant_calls", "Pf_ANC.ASM922.final.tsv"))
pfE1 =
  left_join(read_tsv(here::here(
    "variant_calls", "Pf_E1.ASM922.final.tsv"
  )), read_delim(
    here::here("variant_calls", "Pf_E1.ASM922.hapfreq.tsv"),
    col_names = c(
      "CHROM",
      "FILTER",
      "POS",
...
pfE2 =
  left_join(read_tsv(here::here(
    "variant_calls", "Pf_E2.ASM922.final.tsv"
  )), read_delim(
    here::here("variant_calls", "Pf_E2.ASM922.hapfreq.tsv"),
    col_names = c(
      "CHROM",
      "FILTER",
      "POS",
...
pfE3 =
  left_join(read_tsv(here::here(
    "variant_calls", "Pf_E3.ASM922.final.tsv"
  )), read_delim(
    here::here("variant_calls", "Pf_E3.ASM922.hapfreq.tsv"),
    col_names = c(
      "CHROM",
      "FILTER",
      "POS",
...
As supplementary dataset
```{r}
df.final = bind_rows(pfE1, pfE2, pfE3) %>%
  anti_join(., dplyr::select(pfanc, CHROM, POS)) %>% # exclude calls present in ancestor strain
  ...
  select(SPECIES, REPLICATE=replicate, POS, REF, ALT, FILTER, ALLELENUM, GENOTYPE=GT, REF.FREQ, ALT.FREQ, PHASEGROUP, HAP1.FREQ, HAP2.FREQ, HAP3.FREQ, FTYPE:PRODUCT) %>%
  arrange(POS)

write_tsv(df.final, here::here("results", "final_variants.tsv"))
```
# MULTIPLICITY
Crude first look at which genes are mutated in more than one replicate population
```{r}
mult1 = df.final %>%
...
```{r}
mutations_survive.plot = ggplot(allele_mult, aes(x=mi, y=value, color=name)) + 
  geom_step() +
...
```{r}
lns = read_tsv(here::here("genome", "PsFluSBW25", "gene_lengths.tsv"), col_names=c("LOCUS_TAG", "length"))
```

```{r}
obs_gene_multiplicity = df.final %>%
...
Now we want to check whether this G score value is statistically significant so we use permutation tests

required input stats about the different populations
```{r}
df.pop.summary = df.final %>%
  dplyr::filter(!is.na(LOCUS_TAG)) %>% 
  #dplyr::filter(!str_detect(EFFECT, "synonymous_variant")) %>%
  count(REPLICATE) %>%
  mutate(sum=sum(n), p=n/sum)

nmutations       = df.pop.summary$sum[1]
population_num   = nrow(df.pop.summary)
population_probs = pull(df.pop.summary, p)
...
plot
```{r}
G_score.plot = ggplot(tibble(g_reps), aes(x=g_reps)) + 
  geom_histogram() +
...
```{r}
t = calculate_unnormalized_survival_from_vector(pooled_pvalues)

observed_ps = t[[1]]
observed_pvalue_survival = t[[2]]
```

Estimate final survival function for genes
```{r}
nglpsf.out = NullGeneLogpSurvivalFunction(obs_gene_multiplicity, rep(0:399), 0.1)
```
...
df2plot = tibble(
  obs_p = observed_ps,
  Survival_Expected = nglpsf.out$survivals,
...
segs = tibble(
  x = c(min(observed_ps), nglpsf.out$pstar),
  y = c(nglpsf.out$num_significant, 0),
  xend = c(nglpsf.out$pstar, nglpsf.out$pstar),
  yend = c(nglpsf.out$num_significant, nglpsf.out$num_significant)
)

anns = tibble(x = nglpsf.out$pstar,
              y = nglpsf.out$num_significant)

```

Survival function plot
```{r}
genes_survive.plot = ggplot() +
  geom_segment(
    data = segs,
    aes(
      x = x,
...
pfinal = mutations_survive.plot + G_score.plot + genes_survive.plot + plot_annotation(tag_levels = 'A')
pfinal
```

```{r}
ggsave(here::here("figs", "multiplicity.svg"), pfinal, device="svg", width=7, height=2.5, units="in")
```

Which genes have more parallel mutations than expected by chance?
```{r}
gene_table_out <- data.frame(cbind(pooled_pvalues, nglpsf.out$survivals)) %>%
  rownames_to_column(var="LOCUS_TAG") %>%
  tibble() %>%
  left_join(., obs_gene_multiplicity) %>%
  filter(pooled_pvalues >= nglpsf.out$pstar) %>%
  left_join(., distinct(select(df.final, LOCUS_TAG, GENE, PRODUCT))) %>%
...
write_tsv(gene_table_out, here::here("results", "significant_genes.tsv"))
...
```{r}
path2ko = readr::read_tsv(here("gene_annotations", "kegg", "path2ko.tsv"), 
                           col_names = c("PATH", "KO"), 
                           col_types="cc")
pathdesc = readr::read_tsv(here("gene_annotations", "kegg", "path_desc.tsv"), 
...
```{r}
eggnog = read_tsv(here("gene_annotations", "SBW25.annotations"),
...
GENES = names(pooled_pvalues[pooled_pvalues > nglpsf.out$pstar])
